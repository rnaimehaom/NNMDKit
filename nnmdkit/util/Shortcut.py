...
def write_data(smiles, mw, ntotal, density, output_dir, output_prefix='system', tmp_ff='opls-aa', terminator='*[H]', cleanup=True):
    ...
    with open('{}.data'.format(output_prefix), 'rt') as lines:
        for line in lines:
            ...
            if 'Bonds' in line:
                break
            if readheader and not any(x in line for x in ['bond', 'angle', 'dihedral', 'improper']):
...
def write_lammps_input(Tinit, Tfinal, Tinterval, step, NN_POTENTIAL, output_dir, datafile_prefix='system', pair_style='nn', element='C H'):
    ...
    with open(output_dir + '/' + settings_fname, 'w') as f:
        ...
        f.write('{:<15} * * {} {}\n'.format('pair_coeff', NN_POTENTIAL, element))
        f.write('\n')
        f.write('{:<15} {} bin\n'.format('neighbor', neighbor_skin))
        f.write('{:<15} delay 0 every {} check yes\n'.format('neigh_modify', neighbor_every))
        f.write('\n')
        f.write('{:<15} custom step temp density vol press ke pe ebond evdwl ecoul elong\n'.format('thermo_style'))
    ...
    with open(output_dir + '/' + lmp_input_fname, 'w') as f:
        ...
        f.write('# LAMMPS input file generated by high-throughput Polymer MD generator\n')
        f.write('\n')
        ...
        f.write('{:<15} dump1 all custom 10000 equil.lammpstrj id mol type q xs ys zs ix iy iz\n'.format('dump'))
        f.write('{:<15} {} equilibrated.restart\n'.format('restart', totaltime_eq))
        f.write('\n')

        for n, i in enumerate(eq_step):
            if i[0] == 'nvt':
                f.write('{:<15} step{} all nvt temp {} {} {}\n'.format('fix', n+1, i[2], i[2], Tdamp))
            elif i[0] == 'npt':
                f.write('{:<15} step{} all npt temp {} {} {} iso {} {} {}\n'.format('fix', n+1, i[2], i[2], Tdamp, i[3], i[3], Pdamp))
            f.write('{:<15} {}\n'.format('run', i[1]))
            f.write('{:<15} step{}\n'.format('unfix', n+1))
            f.write('\n')
        f.write('{:<15} dump1\n'.format('undump'))
        ...
        f.write('### Production\n')
        f.write('{:<15} dump2 all custom 10000 production.lammpstrj id mol type q xs ys zs ix iy iz\n'.format('dump'))
        f.write('{:<15} {} production.restart\n'.format('restart', step))        
        f.write('{:<15} Rho equal density\n'.format('variable'))
        f.write('{:<15} Temp equal temp\n'.format('variable'))
        f.write('{:<15} fDENS all ave/time {} {} {} v_Temp v_Rho file temp_vs_density\n'.format('fix', int(step/100/4), 100, step))
        f.write('\n')
        
        f.write('{:<15} loop\n'.format('label'))
        f.write('{:<15} a loop {}\n'.format('variable', int((Tinit-Tfinal)/Tinterval+1)))
        f.write('{:<15} b equal {}-{}*($a-1)\n'.format('variable', Tinit, Tinterval))
        f.write('{:<15} fNPT all npt temp $b $b {} iso {} {} {}\n'.format('fix', Tdamp, Pinit, Pinit, Pdamp))
        f.write('{:<15} {}\n'.format('run', step))
        f.write('{:<15} fNPT\n'.format('unfix'))
        f.write('{:<15} a\n'.format('next'))
...
def write_pbs(jobname, nodes, ppn, walltime, LAMMPS_EXEC, output_dir, project='GT-rramprasad3-CODA20'):
    pbs_fname = 'job.pbs'
    with open(output_dir + '/' + pbs_fname, 'w') as f:
        ...
        f.write('mpirun -np {} {} -in lmp.in\n'.format(int(nodes*ppn), LAMMPS_EXEC))
...
if __name__ == '__main__':
    ...
    for s in smiles:
        ...
        write_lammps_input(Tinit=600, Tfinal=100, Tinterval=25, step=1000000, NN_POTENTIAL='~/p-rramprasad3-0/NNLMP/potential_saved', output_dir=s)
        write_pbs(jobname=s, nodes=2, ppn=24, walltime='48:00:00', LAMMPS_EXEC='~/p-rramprasad3-0/NNLMP/lmp', output_dir=s)